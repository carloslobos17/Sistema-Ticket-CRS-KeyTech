                ...
                <div className="overflow-x-auto bg-white rounded-xl shadow-sm border border-gray-200">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {tickets
                                // Filtramos aquí por si acaso para no mostrar "Cerrados"
                                .filter(t => t.status?.name !== 'Cerrado')
                                .map((ticket) => {
                                    // Definimos los estilos de estado como en index.jsx
                                    const statusName = ticket.status?.name || "Sin estado";
                                    const statusStyles = {
                                        "Pendiente a asignación": "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400",
                                    ...
                                    return (
                                        <tr key={ticket.id}>
                                            ...
                                            {/* Columna de Técnico Asignado */}
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {ticket.assigned_user?.name || (ticket.assigned_user ? 'Cargando...' : 'Sin asignar')}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex gap-2">
                                                    ...
                                                    >
                                                        {ticket.assigned_user ? "Reasignar" : "Asignar"}
                                                    </Button>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex gap-2">
                                                    {/* Cambiamos el Button por un Link que redirija a la vista Show */}
                                                    <Button
                                                        size="sm"
                                                        className="bg-red-600 hover:bg-red-700"
                                                        asChild // Esto permite usar Link como hijo de Button
                                                    >
